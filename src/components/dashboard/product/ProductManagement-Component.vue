...
export default {
  ...
  methods: {
    fetchData() {
      ...
      axios.get(`${ApiUrl}/products/fetch`).then((res) => {
        ...
        //console.log("Customer : " + this.products[0].username);
      })
          .catch((errors) => {
    ...
    sendProduct(){
      ...
      const formData = new FormData()
      formData.append('token', token)
      formData.append('price', this.productSend.price)
      formData.append('name',  this.productSend.name )
      formData.append('description', this.productSend.description)
      formData.append('imageUrl', this.productSend.imageUrl)

      formData.append('stock', this.productSend.stock)
      formData.append('discount', this.productSend.discount)

      axios.post(`${ApiUrl}/products/create`, formData, {}).then((res) => {
        ...
        if (res.data.status === "error") {
          ...
          Swal.fire({
            icon: 'success',
            title: 'Le produit a bien été ajouter',
            showConfirmButton: false,
            background:"#181818",
            timer: 1500
          })
        }
        this.productSend.price = ""
        this.productSend.name = ""
        this.productSend.description = ""
        this.productSend.imageUrl = ""
        this.productSend.stock = ""
        this.productSend.discount = ""
      })
    },
    deleteProduct(id , name){
      ...
      Swal.fire({
        ...
        denyButtonText: `Non j'ai changer d'avis`
      }).then((result) => {
        /* Read more about isConfirmed, isDenied below */
        if (result.isConfirmed) {
          Swal.fire("Produit supprimer!", "", "success");
          axios.post(`${ApiUrl}/products/delete`, formData, {}).then((res) => {
            if (res.error === "Bad Request"){
              Swal.fire({
                title: 'Erreur',
                text: "Le formulaire n'est pas conforme",
            ...
            if (res.data.status === "error") {
              ...
              Swal.fire({
                icon: 'success',
                title: 'Le produit a bien été supprimer',
                showConfirmButton: false,
                background:"#181818",
                timer: 1500
              })
          ...
          Swal.fire("Suppression annuler", "", "error");
        }
      });
    },
...
<template>
  <h1 class="text-2xl text-center"> ProductManagement </h1>
    ...
    <div
      ...
      <div class="w-full mb-1">
        <div class="mb-4 ">
          ...
          <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white text-center">All products</h1>
        </div>
        ...
        <div class="inline-block min-w-full align-middle">
          <div class="overflow-hidden shadow">
            <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
              ...
              <tbody v-for="product in products" class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
              <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                ...
                <td class="p-4 space-x-2 whitespace-nowrap">
                  ...
                  <button v-on:click="deleteProduct(product.id, product.name)" type="button" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-red-700 rounded-lg hover:bg-red-800 focus:ring-4 focus:ring-red-300 dark:focus:ring-red-900">
    ...
    <div id="drawer-create-product-default"
      ...
      <form @submit.prevent="sendProduct">
        <div class="space-y-4">
          <div>
            <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Name</label>
            <input type="text" name="title" id="name" v-model="productSend.name"
                   class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
          ...
          <div>
            <label for="price" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Price</label>
            <input type="number" name="price" id="price" v-model="productSend.price"
                   class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                   placeholder="$2999" required="">
          </div>
          <div>
            <label for="price" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Stock</label>
            <input type="number" name="price" id="price" v-model="productSend.stock"
                   class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                   required="">
          </div>
          <!--<div>
          ...
          <div>
            <label for="description"
                   class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Description</label>
            <textarea id="description" rows="4" v-model="productSend.description"
                      class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
          ...
          <div>
            ...
            <select id="discount-create" v-model="productSend.discount"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
              <option v-for="n in 100" selected="">{{n}}%</option>
            </select>
          </div>
